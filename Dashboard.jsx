...
interface Alert {
  ...
  message: string;
  action: string;
  type: "spike" | "threshold" | "info" | "disconnect";
}
...
const Gauge: React.FC<GaugeProps> = ({
  ...
  // Prevent negative values for calculation
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-gray-800/50 rounded-lg border border-gray-700 relative h-48 transition-all hover:bg-gray-800/70">
      ...
      <span className="text-sm text-gray-300 mt-[-20px] font-medium tracking-wider">
        {label}
      </span>
    </div>
  );
      ...
      <div className="w-full overflow-hidden">
        ...
        >
          <path
            ...
            fill="none"
            className={${color} opacity-20 fill-current}
          />
          <path
            d={M ${points}}
            fill="none"
...
const StationMap: React.FC<StationMapProps> = ({
  ...
  return (
    <div
      className={bg-gray-800/30 border border-gray-700 rounded-lg p-6 relative overflow-hidden transition-all ${className}}
    >
      ...
      <div className="relative w-full h-full">
        {stations.map((station) => (
          <div
            ...
            style={{
              left: ${Math.min(Math.max(station.x, 10), 90)}%,
              top: ${Math.min(Math.max(station.y, 10), 90)}%,
            }}
          >
            <div
...
export default function App() {
  ...
  const [region, setRegion] = useState<string>("Delhi-NCR");
  const [timeRange, setTimeRange] = useState<string>("24 Hours");

  const regionBaselines: { [key: string]: RegionBaseline } = {
  ...
  const [climateData, setClimateData] = useState<ClimateData>({
    ...
    co2: 410,
    history: [
      140, 142, 145, 148, 150, 155, 152, 148, 145, 142, 140, 138, 145, 150, 155,
    ],
  });
  ...
  const alertsList: Alert[] = [
    {
      id: 1,
      severity: "CRITICAL",
      timestamp: "2025-11-20 10:32 AM",
      station: S001 (${region}),
      message: AQI spiked to ${Math.floor(climateData.aqi * 1.2)} (Hazardous),
      action: "View Graph",
      type: "spike",
    },
    {
      ...
      station: S003 (${region}),
      message: "NO2 levels above safe threshold",
      action: "View Graph",
      type: "threshold",
    },
    {
      ...
      station: "System",
      message: "Daily calibration completed successfully",
      action: "Archived",
      type: "info",
    },
    {
      id: 4,
      severity: "CRITICAL",
      timestamp: "2025-11-19 11:45 PM",
      station: S001 (${region}),
      message: "Sensor disconnect detected",
      action: "Diagnose",
      type: "disconnect",
    },
  ];
  ...
  useEffect(() => {
    const baseline = regionBaselines[region] || regionBaselines["Delhi-NCR"];
    setClimateData((prev) => ({
      ...prev,
      aqi: baseline.aqi,
      temp: baseline.temp,
      ...
      history: Array(15)
        .fill(0)
        .map(() => Math.max(0, baseline.aqi + (Math.random() * 20 - 10))),
    }));
    ...
    const interval = setInterval(() => {
      setClimateData((prev) => {
        ...
        const newAqi = Math.min(
          Math.max(prev.aqi + (Math.random() * volatility - volatility / 2), 0),
          500
        );
        return {
          aqi: Math.floor(newAqi),
          temp: parseFloat(
            (prev.temp + (Math.random() * 0.5 - 0.25)).toFixed(1)
          ),
          humidity: Math.min(
            Math.max(Math.floor(prev.humidity + (Math.random() * 4 - 2)), 0),
            100
          ),
...
[DIGITAL SIGNATURE VERIFIED]
      ...
      a.href = url;
      a.download = Incident_Report_${selectedAlert.id}_${Date.now()}.txt;
              ...
              <div>
                ...
                <div className="space-y-2 font-mono text-xs bg-black/50 p-4 rounded border border-gray-800">
                  <div className="text-green-400">
                    {">"} Ping S001... Unreachable
                  </div>
                  <div className="text-green-400">
                    {">"} Check Power Rail... Nominal (5.0V)
                  </div>
                  <div className="text-red-400 animate-pulse">
                    {">"} Wireless Uplink... Failed (No Handshake)
                  </div>
                  <div className="text-gray-500">
                    {">"} Retrying connection (Attempt 1/3)...
                  </div>
                  <div className="text-yellow-400 mt-2">
                    {">"} Recommendation: Field technician required for module
                    reset.
                  </div>
                </div>
              </div>
  ...
  const renderDashboard = () => (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Gauge
          value={climateData.aqi}
          max={500}
          label="Live AQI Index"
          unit="US AQI"
          color={getAqiColor(climateData.aqi)}
        />
        <Gauge
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Sparkline
          ...
          value={(climateData.aqi * 0.6).toFixed(1)}
          unit="µg/m³"
        ...
        <Sparkline
          ...
          value={(climateData.aqi * 0.9).toFixed(1)}
          unit="µg/m³"
          data={climateData.history.map((x) => x * 1.2)}
        ...
        <Sparkline
          ...
          value="42"
          unit="ppb"
          data={climateData.history.map((x) => Math.max(x - 100, 10))}
          color="stroke-blue-500"
        />
        <Sparkline
          label="Ozone (O3)"
          value="36"
          unit="ppb"
          data={climateData.history.map((x) => x / 3)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-800/30 p-4 rounded border border-gray-700/50">
          ...
          <div className="grid grid-cols-2 gap-y-2 text-sm">
            ...
            <div className="text-gray-300">IoT-Pro-Max (2023)</div>
            <div className="text-gray-500">Zone Type:</div>
            <div className="text-gray-300">Industrial / High Traffic</div>
            <div className="text-gray-500">Data Interval:</div>
            <div className="text-gray-300">{timeRange} Avg</div>
          </div>
        </div>
        <div className="bg-gray-800/30 p-4 rounded border border-gray-700/50">
          <h3 className="text-sm font-bold text-gray-400 mb-2 uppercase">
  ...
  const renderSources = () => (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-white mb-4">
        Pollution Source Apportionment ({region})
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-400">
            High traffic volume on Ring Road affecting sensors.
          </p>
        ...
        <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-400">
            Okhla Industrial Area plume detected.
          </p>
          <div className="w-full bg-gray-700 h-2 mt-4 rounded-full overflow-hidden">
            <div
        ...
        <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-400">
            Seasonal crop residue burning in neighboring states.
          </p>
          <div className="w-full bg-gray-700 h-2 mt-4 rounded-full overflow-hidden">
    ...
    <div className="min-h-screen bg-[#0f1218] text-white font-sans selection:bg-cyan-500 selection:text-black relative">
      ...
      <nav className="h-14 border-b border-gray-800 flex items-center justify-between px-4 bg-[#1a1f29]">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-6 h-6 bg-green-500 rounded md:rounded-sm flex items-center justify-center font-bold text-xs text-black">
              C
            </div>
      ...
      <div className="flex h-[calc(100vh-3.5rem)]">
        ...
        <main className="flex-1 p-6 overflow-y-auto bg-gradient-to-br from-[#0f1218] to-[#151921]">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-gray-500 mt-1 flex space-x-2">
                <span>Asia</span> <span>&gt;</span> <span>India</span>{" "}
                <span>&gt;</span>{" "}
                <span className="text-blue-400 capitalize">{region}</span>
              </div>
            </div>
            <div className="flex space-x-2">
              ...
              <button className="px-3 py-1 bg-gray-800 border border-gray-700 text-xs text-gray-300 rounded hover:bg-gray-700">
                Export CSV
              </button>
            </div>
          </div>
          ...
          {activeView === "map" && (
            <StationMap filter={searchQuery} region={region} />
          )}
